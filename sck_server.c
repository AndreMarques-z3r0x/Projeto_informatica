...
int* start_unloads(int zone, int *arr){
    ...
    // int dados[] = {0,0,0,0,0,0,0,0,0};
    // dados[arr[2]-1] = arr[1];
    // if(arr[0] = 1){
    //     for(int k= 0; k<9; k++){
    //         printf("[+]dados[%d] --> %d",k, dados[k] );
    //     }
    //     plc_write_values("unload_zone_1", 1,9 , dados);
    // } else if(arr[0] = 2){
    //     plc_write_values("unload_zone_2", 1,9 , dados);
    // }else if (arr[0] = 3){
    //     plc_write_values("unload_zone_3", 1,9 , dados);
    // }
    //opcua_connect_to_server();
    return arr;
}
...
void* unloads_thread(void *param){
    ...
    if (zone==1){
        ...
        plc_write_values("unload_zone_2",1,9,dados_sos);
    }else if(zone==3){
        plc_write_values("unload_zone_3",1,9,dados_sos);
    }   
    UA_Client_delete(client);
    return NULL;
}
...
{   
    ...
    while(1){
        ...
        if(strcmp(command,"NewUnload") ==0){
            ...
            *arg = zone;
            pthread_create(&th1, NULL, unloads_thread, arg);
